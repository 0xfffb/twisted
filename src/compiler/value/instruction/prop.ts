...
export class GetPropInstruction extends Instruction {
	...
	dump(): string { return `%${this.id} = getprop ${this.obj}, "${this.key}"`; }
}
...
export class GetElemInstruction extends Instruction {
	...
	dump(): string { return `%${this.id} = getelem ${this.obj}, ${this.key}`; }
}

export class SetPropInstruction extends Instruction {
	readonly kind = "SetProp" as const;
	constructor(id: number, public readonly obj: Value, public readonly key: string, public readonly value: Value) {
		super(id);
	}
	dump(): string { return `setprop ${this.obj}, "${this.key}", ${this.value}`; }
}

export class SetElemInstruction extends Instruction {
	readonly kind = "SetElem" as const;
	constructor(id: number, public readonly obj: Value, public readonly key: Value, public readonly value: Value) {
		super(id);
	}
	dump(): string { return `setelem ${this.obj}, ${this.key}, ${this.value}`; }
}
